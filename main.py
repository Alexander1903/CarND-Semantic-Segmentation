...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    output_7 = tf.layers.conv2d_transpose(conv1x1_7, num_classes, 4, strides=(2, 2), padding='same',
                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    conv1x1_4 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, padding='same',
                               kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    skip_1 = tf.add(output_7, conv1x1_4)
    output_4 = tf.layers.conv2d_transpose(skip_1, num_classes, 4, strides=(2, 2), padding='same',
                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    conv1x1_3 = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, padding='same',
                               kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    skip_2 = tf.add(output_4, conv1x1_3)
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for i in range(epochs):
        ...
        for image, label in get_batches_fn(batch_size):
            loss = sess.run([train_op, cross_entropy_loss], feed_dict={input_image: image, keep_prob: 0.6,
                                                                       learning_rate: 1e-5,
                                                                       correct_label: label})
